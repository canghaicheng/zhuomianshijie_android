...
class TtsSettingsController(private val context: Context) : NanoHTTPD(8081) {
    private val ttsSettingsManager = TtsSettingsManager(context)
        ...
        return when (session.uri) {
            "/tts/settings" -> {
                when (session.method) {
                    Method.GET -> {
                        try {
                            val jsonResponse = ttsSettingsManager.handleTtsSettingsRequest()
