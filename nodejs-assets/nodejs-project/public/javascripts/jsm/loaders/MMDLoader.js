...
class MaterialBuilder {
	...
	/**
	 * 加载纹理并缓存
	 * @param {string} filePath - 纹理文件的路径
	 * @param {Object} textures - 用于缓存已加载纹理的对象
	 * @param {Object} params - 额外的参数对象
	 * @param {Function} onProgress - 加载进度回调函数
	 * @param {Function} onError - 错误处理回调函数
	 * @returns {Texture} 加载的纹理对象
	 */
	_loadTexture( filePath, textures, params, onProgress, onError ) {
		// params 参数是可选的
		params = params || {};

		// 保存 this 引用，以便在嵌套函数中使用
		const scope = this;

		// 声明完整路径变量
		let fullPath;

		// 如果是默认的 toon 纹理，使用预定义的路径
